...
function saveToHistory(type, data) {
    if (!currentUser) {
        console.log('Не удалось сохранить в историю: пользователь не авторизован');
        return;
    }
    ...
    console.log('Сохранено в историю:', type, data);
}
...
function showHistory() {
    ...
    if (history.length === 0) {
        ...
        historyContent.innerHTML = history.slice().reverse().map((item, index) => {
            ...
            switch (item.type) {
                case 'matrix':
                    ...
                    contentHtml = `<strong>${item.data.name || 'Аноним'}</strong><br>Аркан рождения: ${item.data.birthArcane}<br>Аркан судьбы: ${item.data.destinyArcane}<br>Аркан таланта: ${item.data.talentArcane}`;
                    break;
                case 'cardOfDay':
                    typeLabel = '🎴 Карта дня';
                    contentHtml = `<strong>${item.data.card}</strong><br>${item.data.date}<br>Совет: ${item.data.advice}`;
                    break;
                case 'majorArcana':
                    typeLabel = '🎴 Расклад "3 карты"';
                    contentHtml = `<strong>Вопрос:</strong> "${item.data.question}"<br><strong>Карты:</strong> ${item.data.cards.join(' → ')}<br><strong>Ответ:</strong> ${(item.data.answer || '').substring(0, 100)}...`;
                    break;
                default:
                    typeLabel = '📜 Запись';
