...
function showNatalForm() {
    ...
    const form = document.getElementById('natalFormContainer');
    if (form) form.style.display = 'block';
    if (typeof updatePartnerFields === 'function') updatePartnerFields();
}
...
function calculateAstrology() {
    ...
    if (calcType === 'natal') {
        if (natalTab) natalTab.innerHTML = generateNatalResult(name, date, time, place);
        if (transitsTab) transitsTab.innerHTML = '';
        if (synastryTab) synastryTab.innerHTML = '';
        if (typeof showResultTab === 'function') showResultTab('natal');
    } else if (calcType === 'transits') {
        if (transitsTab) transitsTab.innerHTML = generateTransitsResult(name, date);
        if (natalTab) natalTab.innerHTML = '';
        if (synastryTab) synastryTab.innerHTML = '';
        if (typeof showResultTab === 'function') showResultTab('transits');
        ...
        if (synastryTab) synastryTab.innerHTML = generateSynastryResult(name, date, partnerName, partnerDate);
        if (natalTab) natalTab.innerHTML = '';
        if (transitsTab) transitsTab.innerHTML = '';
        if (typeof showResultTab === 'function') showResultTab('synastry');
    }
...
function showResultTab(tabName) {
    ...
    if (natalTab) natalTab.style.display = 'none';
    if (transitsTab) transitsTab.style.display = 'none';
    if (synastryTab) synastryTab.style.display = 'none';
    
    if (tabName === 'natal') {
        if (natalTab) natalTab.style.display = 'block';
        if (tabs[0]) tabs[0].classList.add('active');
    } else if (tabName === 'transits') {
        if (transitsTab) transitsTab.style.display = 'block';
        if (tabs[1]) tabs[1].classList.add('active');
    } else if (tabName === 'synastry') {
        if (synastryTab) synastryTab.style.display = 'block';
...
function getCompatibility(zodiac1, zodiac2) {
    const compatibilities = {
        'Овен-Овен': { score: 85, description: 'Яркий союз двух лидеров. Много страсти, но и конкуренции тоже.', advice: 'Учитесь сотрудничать, а не соревноваться.' },
        'Овен-Телец': { score: 70, description: 'Огонь и Земля: импульсивность встречает стабильность.', advice: 'Овну нужно быть терпеливее, Тельцу — смелее.' },
        'Овен-Близнецы': { score: 90, description: 'Два энергичных знака. Вместе вам никогда не бывает скучно.', advice: 'Поддерживайте общие проекты и приключения.' },
        'Овен-Рак': { score: 55, description: 'Сложное сочетание: прямолинейность Овна ранит чувствительного Рака.', advice: 'Раку — говорить о чувствах, Овну — быть мягче.' },
        'Овен-Лев': { score: 95, description: 'Огненный союз. Взаимное восхищение и поддержка.', advice: 'Не конкурируйте за лидерство, разделите роли.' },
        'Овен-Дева': { score: 60, description: 'Разные ритмы жизни: Овен спешит, Дева все проверяет.', advice: 'Деве — отпустить контроль, Овну — замедлиться.' },
        'Овен-Весы': { score: 75, description: 'Противоположности притягиваются. Дополняете друг друга.', advice: 'Цените различия, они ваша сила.' },
        'Овен-Скорпион': { score: 80, description: 'Сильные личности. Глубокие чувства и страсть.', advice: 'Доверяйте друг другу, не играйте в игры.' },
        'Овен-Стрелец': { score: 95, description: 'Идеальные партнеры для приключений. Оптимизм и энергия.', advice: 'Путешествуйте вместе, это вас укрепляет.' },
        'Овен-Козерог': { score: 50, description: 'Разные цели: Овен хочет сейчас, Козерог строит на будущее.', advice: 'Найдите общие долгосрочные цели.' },
        'Овен-Водолей': { score: 85, description: 'Новаторский союз. Оба ценят свободу и независимость.', advice: 'Давайте друг другу пространство.' },
        'Овен-Рыбы': { score: 65, description: 'Овен — действие, Рыбы — мечты. Может быть красиво, но непрактично.', advice: 'Рыбам — проявлять инициативу, Овну — слушать.' }
    };
